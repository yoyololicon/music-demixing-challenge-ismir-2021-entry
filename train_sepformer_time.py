...
device_trsfm = nn.Sequential(module_transform.SpeedPerturb()).to(device)
...
def process_function(engine, batch):
    ...
    with amp.autocast(enabled=amp_enabled):
        mask, Y_hat = model(X_mag)
        Y_hat = Y_hat.permute(0, 3, 1, 2)
    loss, extra_losses = criterion(Y_hat, y, x)
...
def evaluate_function(engine, batch):
    ...
    with torch.no_grad():
        ...
        #Y = mwf(pred_mask, X)
        xpred = Y_hat.contiguous() #spec(Y, inverse=True)
...
def predict_samples(engine):
    ...
    with torch.no_grad():
        ...
        X_mag = X.abs()
        with amp.autocast(enabled=amp_enabled):
            mask, xpred = model(X_mag.unsqueeze(0))
            xpred = xpred.permute(0, 3, 1, 2)
            xpred = xpred.squeeze()
